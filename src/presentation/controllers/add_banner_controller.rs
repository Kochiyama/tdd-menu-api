...
impl<'a> Controller<AddBannerDto, (), (), Banner> for AddBannerController<'a> {
    fn handle(&self, request: HttpRequest<AddBannerDto, (), ()>) -> HttpResponse<Banner> {
        println!("request: {:#?}", request);

        match self.add_banner.execute(request.body.file_name) {
...
mod tests {
    ...
    fn should_call_add_banner_with_correct_values() {
        ...
        add_banner_mock
            .expect_execute()
            .with(eq(request.body.file_name.clone()))
            .times(1)
